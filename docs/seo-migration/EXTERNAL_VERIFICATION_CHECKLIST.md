...
2. Trigger phone, email and form-submit interactions in a test session.
3. Verify `phone_call_click`, `email_click` and `generate_lead` in DebugView or Realtime with page path, source page, enquiry type, product slug/range/category and attribution fields.
4. Confirm free-text message, name, email, phone and suburb are absent from event parameters.
